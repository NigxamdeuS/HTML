...
<div id="wrapper">
  <h2>JavaScript基礎と主要メソッド</h2>
  <p class="mainp">JavaScriptの基本的な概念から実用的なメソッドまで、体系的にまとめた一覧です。変数宣言、データ型、制御構造、関数、配列操作、DOM操作など、Web開発に必要な要素を網羅しています。初心者の学習から実務での参照まで活用できる内容となっています。</p>

  <h1>基本概念</h1>
  <table>
    <tr>
      <td>変数宣言</td>
      <td>let, const, var を使用して変数を宣言</td>
    </tr>
    <tr>
      <td>データ型</td>
      <td>String, Number, Boolean, Object, Array, null, undefined</td>
    </tr>
    <tr>
      <td>演算子</td>
      <td>算術(+,-,*,/), 比較(==,===,!=,!==), 論理(&&,||,!)</td>
  ...
  <table>
    <tr>
      <td>push(), pop()</td>
      <td>配列の末尾に要素を追加/削除</td>
    </tr>
    <tr>
      <td>map()</td>
      <td>配列の各要素を変換して新しい配列を作成</td>
    </tr>
    <tr>
      <td>filter()</td>
      <td>条件に合う要素だけを抽出して新しい配列を作成</td>
    </tr>
    <tr>
      <td>reduce()</td>
      <td>配列の要素を集計して単一の値を得る</td>
    </tr>
  </table>
  ...
  <table>
    <tr>
      <td>Promise</td>
      <td>非同期処理の結果を扱うオブジェクト</td>
    </tr>
    <tr>
      <td>async/await</td>
      <td>非同期処理を同期的に書けるようにする構文</td>
    </tr>
    <tr>
      <td>fetch()</td>
      <td>HTTPリクエストを送信してデータを取得</td>
    </tr>
  </table>
  ...
  <table>
    <tr>
      <td>Object.keys()</td>
      <td>オブジェクトのキーを配列として取得</td>
    </tr>
    <tr>
      <td>Object.values()</td>
      <td>オブジェクトの値を配列として取得</td>
    </tr>
    <tr>
      <td>Object.entries()</td>
      <td>オブジェクトのキーと値のペアを配列として取得</td>
    </tr>
  </table>
<!--/wrapper--></div>
</main>
